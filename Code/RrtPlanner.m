...
% environment which may of may not take root as trees
    ...
    properties
        ...
        lim=[0.00 +30;0.00 +25 ;0.00 +30];

        % Default Start and Goal  => Point [x y z]
        start=[+0.00 +0.00 +0.00];
        goal =[+12.00 +25.00 +30.00];

        % Number of attempts at path smoothing
        nsmooth=10000;%%%%Cambiato questo,prima era 1000
        ...
        function GenerateObstacles(self)
            ...
            if ~isempty(self.obstacleFilename)
                ...
                for i=1:num_obs
                    ...
                    self.obs(:,:,i)=self.obs(:,:,i)-chage_dim;%In caso di problemi questo � stato aggiunto
                end
